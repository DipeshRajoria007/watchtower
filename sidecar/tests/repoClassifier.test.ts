...
describe('classifyRepo (agent-based)', () => {
  ...
  it('returns the agent-selected repo when confidence clears the threshold', async () => {
    ...
    const out = await classifyRepo({
      texts: ['remove the whatsapp section in the right nav bar on my.newtonschool.co/tech-openings/all-jobs'],
      threshold: 0.75,
    });
  ...
  it('returns newton-api when the agent says so', async () => {
    ...
    const out = await classifyRepo({
      texts: ['the /api/v1/users endpoint returns 500 with a Django traceback'],
      threshold: 0.75,
    });
  ...
  it('marks uncertain when confidence is below threshold', async () => {
    ...
    const out = await classifyRepo({ texts: ['something might be broken'], threshold: 0.75 });
  ...
  it('marks uncertain when the agent returns null', async () => {
    ...
    const out = await classifyRepo({ texts: ['hey'], threshold: 0.75 });
  ...
  it('falls back to uncertain when the agent call fails', async () => {
    ...
    const out = await classifyRepo({ texts: ['anything'], threshold: 0.75 });
    expect(out.uncertain).toBe(true);
    expect(out.selectedRepo).toBeNull();
  });

  it('does not call the agent when there is no text to classify', async () => {
    const out = await classifyRepo({ texts: ['', '   '], threshold: 0.75 });
    expect(out.uncertain).toBe(true);
